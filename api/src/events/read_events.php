...
<body>
    <nav class="navbar navbar-expand-lg">
        <div class="container-fluid">
            <a class="navbar-brand ms-5" href="#">Event <b class="evd">
                    << /b><b class="evd">Dev</b><b class="evd">></b></a>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item">
                        <a class="nav-link" href="../../../eventdev/resources/views/welcome.blade.php"><i
    ...
    <div class="container mt-3">
            ...
            <div class="col-12 col-md-6 col-lg-2">
                <select name="estado" class="form-select">
                    ...
                    <?php foreach ($estados as $estado): ?>
                        <option value="<?php echo htmlspecialchars($estado); ?>"><?php echo htmlspecialchars($estado); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-12 col-md-6 col-lg-2">
                <select name="mes" class="form-select">
                    ...
                    <?php foreach ($meses as $numero => $nome): ?>
                        <option value="<?php echo htmlspecialchars($numero); ?>" <?php if (isset($_GET['mes']) && $_GET['mes'] == $numero)
                               echo 'selected'; ?>>
        ...
        <?php if ($result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()):
                ...
                ?>
                <div class="cardss" data-bs-toggle="modal" data-bs-target="#eventModal"
                    onclick="showEventDetails(<?php echo htmlspecialchars(json_encode($row)); ?>)">
                    <div class="card-headers">
                        <div class="card-titles">
                            <i class="fas fa-calendar-alt"></i>
                            <a
                                class="link-success link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover link">
                                <?php echo htmlspecialchars($row['nome_evento']); ?>
                            </a>
                            <span
                                class="badge <?php echo htmlspecialchars($badge_color); ?>"><?php echo htmlspecialchars($status); ?></span>
                            <span class="alt-span">⚠ clique no card para expandir o evento</span>
                        </div>
                    </div>
                    <hr>
                    <div class="card-info">
                        <div><i class="fa-solid fa-location-dot"></i> <?php echo htmlspecialchars($row['localizacao']); ?></div>
                        <div class="badge-categ"><i class="fa-solid fa-layer-group"></i>
                            <?php echo htmlspecialchars($row['categoria']); ?></div>
                        <div class="cash"><i class="fa-solid fa-sack-dollar"></i>
                            <?php echo htmlspecialchars($row['valor_entrada']); ?></div>
                        <div><i class="fa-regular fa-clock"></i> <?php echo formatDate($row['data_inicio']); ?> -
                            <?php echo formatDate($row['data_fim']); ?></div>
                        <div><i class="fa-solid fa-crown"></i> <?php echo htmlspecialchars($row['organizador']); ?></div>
                        <div><i class="fa-solid fa-tree-city"></i> <?php echo htmlspecialchars($row['cidade']); ?> /
                            <?php echo htmlspecialchars($row['estado']); ?></div>
                    </div>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 style="color: black" class="modal-title" id="eventModalLabel">Detalhes do Evento</h5> <span
                        class="badge <?php echo htmlspecialchars($badge_color); ?>"><?php echo htmlspecialchars($status); ?></span>
